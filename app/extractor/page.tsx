...
//  - [\s-]?  -> tolerates a single space or hyphen between the country
//    code/prefix and the local part, and between digit groups within the
//    local part (e.g. WhatsApp shows "+880 1734-964751"). This only
//    tolerates formatting WITHIN one candidate match; the (?<!\d)/(?!\d)
//    boundary still guarantees we never glue two separate numbers
//    together, since a digit immediately outside the match still blocks it.
...
const EXTRACT_RE = /(?<!\d)(?:\+?880[\s-]?|0)1[3-9](?:[\s-]?\d){8}(?!\d)/g;

function normalize(raw: string): string | null {
  // Strip the formatting characters EXTRACT_RE tolerated (spaces, hyphens,
  // leading +) before validating/storing -- stored numbers are always the
  // plain 8801XXXXXXXXX digit string, regardless of how they were displayed.
...
export default function PhoneExtractorPage() {
  ...
  const initWorker = useCallback(async () => {
    ...
    workerInitPromise.current = (async () => {
      try {
        // Load BOTH English and Bengali language data. With eng-only, the
        // segmenter treats Bengali glyphs as noise and can skip whole
        // lines -- digits included -- on mixed-script screenshots.
        const worker = await createWorker(["eng", "ben"]);

        // IMPORTANT: no tessedit_char_whitelist here on purpose.
        // Whitelisting to digits-only while running a two-language model
        // fights the recognizer: it's trying to predict real Bengali
        // glyphs, then gets forced to squeeze every prediction into just
        // "0-9+", which can suppress whole words instead of cleanly
        // ignoring them. We let Tesseract read the full raw text
        // (Bengali + English, whatever it finds) and let EXTRACT_RE do
        // 100% of the filtering afterward -- that's what it's built for.
        await worker.setParameters({
          // SPARSE_TEXT suits app screenshots where a short number sits
          // in a label:value line among unrelated text, rather than one
          // uniform paragraph block (the default AUTO mode).
